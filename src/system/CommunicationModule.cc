...
std::string CommunicationModule::getMessDirName() {
    return this->mess_dir_name_;
}

uint64_t CommunicationModule::getMessageInStorageCount() {
    return queue_.getAll().size();
}
...
CommunicationModule CommunicationModule::createServer(uint16_t port, std::string mess_dir_name) {
    CommunicationModule result(port, mess_dir_name);
    return result;
}

CommunicationModule
CommunicationModule::createClient(uint16_t server_port, Sockets::IP address, uint16_t port, std::string mess_dir_name) {
    CommunicationModule result(port, mess_dir_name);
...
//    queue_.remove(receivedMess->id_);
    return receivedMess;
}
